...
def define_pretrained_yolo3(yolo_weight, model_def, gpu_ids=[]):
    
    # Initiate model
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model = Darknet(model_def).to(device)
    if yolo_weight.endswith(".weights"):
        # Load darknet weights
        model.load_darknet_weights(yolo_weight)
    else:
        # Load checkpoint weights
        model.load_state_dict(torch.load(yolo_weight))
